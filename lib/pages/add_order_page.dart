  ...
  Future<void> _save() async {
    if (_selectedWarehouseId == null) {
      ScaffoldMessenger.of(context)
          .showSnackBar(const SnackBar(content: Text('请选择仓库')));
      return;
    }
    final partner = _partnerController.text.trim();
    if (partner.isEmpty) {
      ScaffoldMessenger.of(context)
          .showSnackBar(const SnackBar(content: Text('请输入客户名称')));
      return;
    }
        ...
        if (mounted) {
          Navigator.of(context).pushReplacement(MaterialPageRoute(
            builder: (_) => AddOrderItemPage(
                order: mo, existingItems: items, existingFees: fees),
          ));
        }
      }
    } else {
